...
import type { Pill, Prescription } from "@/core/domain/types"
...
const authorDisplay = computed(() => (pill.value ? formatAuthor(pill.value.author_name, pill.value.author_email) : null))
...
async function archivePill() {
    ...
    try {
        await confirm(t("confirm.delete_pill_msg", { title: pill.value.title }), t("confirm.delete_pill_title"), {
...
async function purgePill() {
    if (!pill.value) return
    try {
        await confirm(t("confirm.purge_pill_msg", { title: pill.value.title }), t("confirm.purge_pill_title"), {
            confirmText: t("common.delete_permanent"),
            cancelText: t("common.cancel"),
            waitSeconds: 3,
        })
...
const rawSource = computed(() => pill.value?.content ?? "")
const { copy, copied } = useClipboard({ source: rawSource, copiedDuring: 1500, legacy: true })
</script>
    ...
    <div class="p-6 max-w-4xl mx-auto space-y-5">
        ...
        <template v-else-if="pill">
            <div class="space-y-3">
                        ...
                        <div class="min-w-0">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                <span
                                    ...
                                    class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-(--badge-zinc-bg) text-(--badge-zinc-text)"
                                    >{{ $t("pill_detail.archived_badge") }}</span
                                >
                            </div>
                ...
                <div class="flex gap-2">
                    <template v-if="canEdit">
                        <button
                            ...
                            @click="
                                router.push(
                                    `/bottles/${shortId(props.bottle_id)}/prescriptions/${shortId(props.rx_id)}/pills/${shortId(props.pill_id)}/edit`,
                                )
                            "
                        >
                            {{ $t("common.edit") }}
                ...
                <div
                    v-if="rxClosed && !isArchived"
                    class="flex items-start gap-2 text-sm text-(--callout-warn-text) border border-(--callout-warn-border) bg-(--callout-warn-bg) px-3 py-2 rounded-lg"
                >
            ...
            <div class="space-y-3">
                <div class="flex gap-1 border-b border-(--border)">
                    ...
                    <button
                        ...
                        class="flex items-center gap-1.5 px-3 py-2 text-xs border-b-2 -mb-px transition-colors"
                        :class="
                            view === 'raw' ? 'border-(--text-h) text-(--text-h)' : 'border-transparent text-zinc-500 hover:text-zinc-300'
                        "
                        @click="view = 'raw'"
                    >
                ...
                <div class="relative bg-(--bg-surface) border border-(--border) rounded-lg p-5 pr-12">
                    <button
                        type="button"
                        class="absolute top-3 right-3 flex items-center justify-center size-7 rounded-md border border-(--border) bg-(--bg-surface) text-zinc-400 hover:text-(--text-h) cursor-pointer transition-colors"
                        :title="copied ? $t('common.copied') : $t('common.copy_markdown')"
                        @click="copy(rawSource)"
                    >
                        <ICheck v-if="copied" class="w-3.5 h-3.5 text-green-500" />
                        <IClipboard v-else class="w-3.5 h-3.5" />
                    </button>
